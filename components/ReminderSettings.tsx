...
import type { ReminderSettings as ReminderSettingsType } from '@/lib/reminderService';
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
      ...
      <div className="space-y-6">
        ...
        <div className="pt-4 border-t">
          ...
          <p className="text-sm text-gray-500 mb-2">Reminder akan dikirim ke email anggota yang terdaftar.</p>
        ...
        <div className="pt-2 border-t">
          <h3 className="font-medium mb-3 flex items-center gap-2"><Phone size={16} /> WhatsApp Reminder</h3>
          <p className="text-sm text-gray-500 mb-2">Reminder via WhatsApp (memerlukan API seperti Fonnte/WaGateway).</p>
          <div className="bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded-lg">
            <p className="text-xs text-yellow-800 dark:text-yellow-200">
              💡 Opsional, isi API key di environment variables
            </p>
          </div>
        </div>

        <div className="flex gap-3">
          <button onClick={handleSave} disabled={loading} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2">
            <Save size={16} /> {loading ? 'Menyimpan...' : 'Simpan Pengaturan'}
          </button>
          <button onClick={handleTestReminder} disabled={sending} className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
