  ...
  const fetchAnggotaWithSaldo = async () => {
    ...
    try {
      ...
      for (const docSnap of querySnapshot.docs) {
        const saldoRef = doc(db, 'saldo', docSnap.id);
        const saldoSnap = await getDoc(saldoRef);
        anggotaList.push({
          uid: docSnap.id,
          nama: docSnap.data().nama,
          email: docSnap.data().email,
          saldo: saldoSnap.exists() ? saldoSnap.data().jumlah || 0 : 0,
        });
      }
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => { fetchAnggotaWithSaldo(); fetchData(); }} className="p-2 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700">
          <RefreshCw size={18} />
        </button>
      </div>
      ...
      <div className="mb-4 flex items-center gap-3 flex-wrap">
        ...
        <select
          ...
          onChange={(e) => {
            ...
            if (e.target.value !== 'all') setSelectedAnggota(e.target.value);
          }}
            ...
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Nama Anggota</th>
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Saldo</th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Aksi</th>
                </tr>
              </thead>
                ...
                {anggota.map((a, idx) => (
                  <tr key={a.uid} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-6 py-4 text-sm text-right font-medium text-green-600">
                      Rp {(a.saldo || 0).toLocaleString('id-ID')}
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-6">
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setModalOpen(null)} className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300">Batal</button>
              <button onClick={handleSetor} disabled={submitting} className="flex-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50">
      ...
      {modalOpen === 'tarik' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-6">
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setModalOpen(null)} className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300">Batal</button>
              <button onClick={handleTarik} disabled={submitting} className="flex-1 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 disabled:opacity-50">
