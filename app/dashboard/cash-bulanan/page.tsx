    ...
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={() => { fetchAnggota(); fetchCashData(); }} className="p-2 border rounded-lg hover:bg-gray-50" title="Refresh">
            <RefreshCw size={18} />
          </button>
          {isPengelola && (
      ...
      <div className="flex gap-3 mb-6 bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm">
        <div>
          ...
          <select value={selectedBulan} onChange={(e) => setSelectedBulan(parseInt(e.target.value))} className="px-3 py-2 border rounded-lg dark:bg-gray-700">
        ...
        <div>
          ...
          <select value={selectedTahun} onChange={(e) => setSelectedTahun(parseInt(e.target.value))} className="px-3 py-2 border rounded-lg dark:bg-gray-700">
      ...
      {showSettings && isPengelola && (
        <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border">
          <h3 className="font-semibold mb-3">Atur Besaran Cash</h3>
          <div className="flex gap-3 items-end">
            <div className="flex-1">
              <label className="block text-sm text-gray-600 mb-1">Besaran Cash (Rp)</label>
              <input type="number" value={besaranCash} onChange={(e) => setBesaranCash(parseInt(e.target.value) || 0)} className="w-full px-3 py-2 border rounded-lg dark:bg-gray-700" />
            </div>
                ...
                {anggota.map((a, idx) => {
                  ...
                  return (
                    <tr key={a.uid} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600">Rp {cash?.jumlah?.toLocaleString('id-ID') || besaranCash.toLocaleString('id-ID')}</td>
                      <td className="px-6 py-4">{isPaid ? <span className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded-full bg-green-100 text-green-800"><CheckCircle size={12} /> Lunas</span> : <span className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded-full bg-red-100 text-red-800"><XCircle size={12} /> Belum</span>}</td>
                      <td className="px-6 py-4 text-center">
                        {isPengelola ? (
                          <button onClick={() => handleToggleBayar(a.uid, isPaid ? 'lunas' : 'belum')} disabled={submitting} className={`px-3 py-1.5 rounded-lg text-sm ${isPaid ? 'bg-orange-100 text-orange-700 hover:bg-orange-200' : 'bg-green-100 text-green-700 hover:bg-green-200'}`}>{isPaid ? 'Batalkan' : 'Tandai Lunas'}</button>
                        ) : user?.uid === a.uid && !isPaid ? (
                          <button onClick={handleBayarSendiri} disabled={submitting} className="px-3 py-1.5 bg-green-600 text-white rounded-lg text-sm hover:bg-green-700">Bayar Sekarang</button>
                        ) : user?.uid === a.uid && isPaid ? (
